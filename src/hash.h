...
/** A hasher class for Carrot's 256-bit hash (double SHA-256). */
class CHash256 {
private:
    CSHA256 sha;
public:
...
/** A hasher class for Carrot's 160-bit hash (SHA-256 + RIPEMD-160). */
class CHash160 {
private:
    CSHA256 sha;
public:
...
inline int GetHashSelection(const uint256 PrevBlockHash, int index) {
    ...
    #define START_OF_LAST_7_NIBBLES_OF_HASH 57
    if(index < 7){
	int hashSelection = PrevBlockHash.GetNibble(START_OF_LAST_7_NIBBLES_OF_HASH + index);
    	return(hashSelection);
    }
    int sumOfSeven = 0;
    for(int i = 0; i < 7; i++){
	sumOfSeven += PrevBlockHash.GetNibble(START_OF_LAST_7_NIBBLES_OF_HASH + i);
    }
    if(sumOfSeven < 28){
	return 0;}
    if(sumOfSeven < 39){
...
//	static std::chrono::duration<double>[16];
    ...
    sph_blake512_context     ctx_blake;      //2
    sph_sha512_context       ctx_sha512;     //3
    sph_shabal512_context    ctx_shabal;     //4
    sph_keccak512_context    ctx_keccak;     //5
    sph_shavite512_context   ctx_shavite;    //6
    sph_whirlpool_context    ctx_whirlpool;  //7
    sph_cubehash512_context  ctx_cubehash;   //8
    sph_jh512_context        ctx_jh;         //9
    sph_luffa512_context     ctx_luffa;      //A
    sph_echo512_context      ctx_echo;       //B
    sph_simd512_context      ctx_simd;       //C
    sph_fugue512_context     ctx_fugue;      //D
    sph_groestl512_context   ctx_groestl;    //E
    sph_hamsi512_context     ctx_hamsi;      //F
