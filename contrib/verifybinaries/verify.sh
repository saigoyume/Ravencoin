...
###   carrotcore.org and carrot.org and compares them.
...
WORKINGDIR="/tmp/carrot_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://carrot.org"
HOST2="https://carrot.org"
BASEDIR="/bin/"
VERSIONPREFIX="carrot-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'carrot-',
...
if [ $? -ne 0 ]; then
   echo "carrot.org failed to provide signature file, but carrotcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "carrot.org and carrotcore.org signature files were not equal?"
