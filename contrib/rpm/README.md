...
The RPM spec file provided here is for Carrot-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://carrot.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Carrot git repository](https://github.com/CarrotProject/Carrot) but are not part of
...
Currently only `Source10` is used, for the example `carrot.conf` file.
...
in the carrot git repository. At present only an SVG version of the Carrot
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the carrot
...
needed when 0.12.1 is released, a proper fix is already in the Carrot git
master branch.
...
idea to build Carrot Core against a static version of that library compiled
...
The `carrot.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `carrot-server` package. At this point, most RPM based
...
The `carrot-server` package should have SELinux support. How to properly do
...
Carrot-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/carrot/). He has been
packaging Carrot for Fedora at least since 2012.
...
4. I renamed the `carrot` package that contains the Qt GUI to `carrot-core` as
that appears to be how the general population refers to it, in contrast to
`carrot-xt` or `carrot-classic`. I wanted to make sure the general population
