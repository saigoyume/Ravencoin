...
`test/functional/test_framework/test_framework.py:CarrotTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/carrot_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    enable_carrotd = config["components"].getboolean("ENABLE_RAVEND")

    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/CarrotProject/Carrot/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # https://github.com/CarrotProject/Carrot/pull/5677#issuecomment-136646964
        print("Tests currently disabled on Windows by default. Use --force option to enable")
        sys.exit(0)

    if not (enable_wallet and enable_utils and enable_carrotd):
        print("No functional tests to run. Wallet, utils, and carrotd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[]):
    # Warn if carrotd is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "carrotd"]) is not None:
            print("%sWARNING!%s There is already a carrotd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    if "RAVEND" not in os.environ:
        os.environ["RAVEND"] = build_dir + '/src/carrotd' + exeext
        os.environ["RAVENCLI"] = build_dir + '/src/carrot-cli' + exeext
...
class TestHandler:
    ...
    def __init__(self, num_tests_parallel, tests_dir, tmpdir, test_list=None, flags=None):
        ...
        # In case there is a graveyard of zombie carrotds, we can apply a
...
class RPCCoverage():
    ...
    commands per `carrot-cli help` (`rpc_interface.txt`).
